...
class ONCE(object):
    ...
    def project_lidar_to_image(self, seq_id, frame_id):
        ...
        for cam_no, cam_name in enumerate(self.__class__.camera_names):
            ...
            # Depth values
            depths = points_lidar[:, 2]
            min_d, max_d = np.min(depths), np.max(depths)

            # Normalize depth → intensity (closer = brighter, farther = darker)
            depth_norm = 255 * (1 - (depths - min_d) / (max_d - min_d))
            depth_norm = depth_norm.astype(np.uint8)

            # Make black image (single channel)
            h, w = img_list[cam_no].shape[:2]
            depth_img = np.zeros((h, w), dtype=np.uint8)

            # Draw points with intensity based on depth
            for (point, intensity) in zip(points_img, depth_norm):
                try:
                    cv2.circle(depth_img, (int(point[0]), int(point[1])), 2, int(intensity), thickness=-1)
                except:
                    continue

            points_img_dict[cam_name] = depth_img
